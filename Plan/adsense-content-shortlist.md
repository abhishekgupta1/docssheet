...
Companion to `docssheet-launch-spec.md` / `docssheet-launch-plan.md`. All page
paths below live in the site repo `../abhishekgupta1.github.io`; this `docssheet/`
repo holds only the `Plan/` docs.
...
Privacy). The site already has **163 docs + 44 cheat sheets** — well past the
"sufficient original content" bar — so the risk is not quantity. The risk is a
reviewer landing on a thin, half-finished, or policy-borderline page. This list
is the set of pages to **personally QA and point at** before submitting: the
strongest, most substantive, evergreen, ad-safe content on the site.
...
- [ ] **Broken-link build.** `docusaurus.config.js` currently has
      `onBrokenLinks: 'warn'`. Run `npm run build` and read the warning list;
      fix any broken internal links on shortlisted pages. Consider flipping to
      `'throw'` once clean so regressions fail CI. (Spec R22 / decision D4.)
- [ ] **`docs/intro.md` still says "Abhishek Gupta Knowledge Base"** in the
      title, `<h1>`, description, and the SVG diagram. This is the docs landing
      page — rebrand it to "docssheet" wording for consistency. (Spec R7.)
- [ ] **`google-site-verification` meta is a placeholder**
      ...
      remove the entry — shipping an invalid verification tag on every page is
      sloppy and pointless. (Spec R14 / decision D2.)
- [ ] **AdSense verification snippet is not on the site yet.** `ADS_ENABLED` is
      `false` in `src/data/site.js`, so no AdSense code loads. The new AdSense
      onboarding needs either the `<script>` loader or a
      `<meta name="google-adsense-account" content="ca-pub-…">` tag present
      during review. The tag must carry the account's real `ca-pub-…` ID — the
      `ca-pub-0000000000000000` placeholder will not verify. Add just the
      verification tag (not ad slots) when you start the application — see plan
      Phase 6. (Spec R20 / decision D3.)
- [ ] **`sitemap.xml`** is generated by the classic preset (config present in
      `docusaurus.config.js`). Confirm it lists the shortlisted URLs after a
      build and is reachable at `/sitemap.xml`.
- [ ] **`robots.txt`** (`static/robots.txt`) allows all and points to the
      sitemap. ✅ Already correct.

---

## 6. Not on the shortlist (still fine to keep public)

The MBA / leadership "response library" and "playbook" category pages (~1,800–3,500
words each) are original and policy-safe, but they're the most formulaic on the
site and least likely to impress a reviewer. Leave them published — they add to
the content count — just don't steer a reviewer toward them.
